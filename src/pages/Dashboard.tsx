...
import { type MesaConPedido } from '@/hooks/useAdminWebSocket'
...
const Dashboard = () => {
  ...
  return (
    <div className="w-full max-w-7xl lg:max-w-[1600px] xl:max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 py-4 md:py-6">
      <div className="flex flex-col h-[calc(100dvh-4rem-2rem)] md:h-[calc(100dvh-7rem)] animate-in fade-in duration-500 overflow-hidden">
        {/* Main Content - Mesas */}
                ...
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 pb-4">
                  {sortedMesas.map((mesa) => {
                    ...
                    return (
                      <Card
                        key={mesa.id}
                        className={`transition-all duration-300 hover:shadow-lg cursor-pointer ${hasActiveOrder && !mostrandoCerrado ? 'border-primary/30 shadow-sm' : ''
                          } ${mesa.pedido?.estado === 'preparing' && !mostrandoCerrado ? 'ring-2 ring-primary/20' : ''} ${mostrandoCerrado ? 'border-amber-300 dark:border-amber-700 bg-amber-50/50 dark:bg-amber-950/20' : ''
                          }`}

      ...
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center justify-between gap-4">
          <div className="min-w-0 space-y-2">
            <div className="h-4 w-40 rounded bg-white dark:bg-muted animate-pulse" />
            <div className="h-3 w-56 max-w-full rounded bg-white dark:bg-muted/60 animate-pulse" />
          </div>
          <div className="h-9 w-28 shrink-0 rounded-lg bg-white dark:bg-muted animate-pulse" />
        </div>
      ))}
    </div>
  )

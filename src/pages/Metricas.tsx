...
export default function Metricas() {
  ...
  useEffect(() => {
    const fetchMetricas = async () => {
      ...
      setLoading(true)
      try {
        const response = await metricasApi.get(token) as { success: boolean; data: MetricasData }
        if (response.success) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-4 min-h-dvh bg-zinc-50 dark:bg-zinc-950">
        <div className="text-center flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-zinc-500 font-medium">Calculando métricas del mes...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-dvh bg-zinc-50 dark:bg-background pb-24 selection:bg-[#FF7A00]/20 selection:text-[#FF7A00] overflow-x-hidden">
      ...
      <div className="bg-white dark:bg-zinc-950/50 border-b border-zinc-200 dark:border-zinc-800/80 pb-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-8 mt-12 relative z-10">
          ...
          <p className="text-muted-foreground mt-2 font-medium">Rendimiento financiero y operativo del mes actual.</p>
        </div>
      </div>
        ...
        <div className={`${phantomCardClass} p-6 sm:p-8 relative overflow-hidden bg-linear-to-br from-white to-zinc-50 dark:from-zinc-950 dark:to-black`}>
          ...
          <div className="flex flex-col mb-8 relative z-10">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Facturado este mes</span>
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-zinc-600 dark:text-zinc-400 font-medium">
              <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-xs"></div>Efectivo ({Math.round(pctEfectivo)}%)</div>
              {pctTarjeta > 0 && <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-pink-500 shadow-xs"></div>Tarjeta ({Math.round(pctTarjeta)}%)</div>}
              <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-sky-500 shadow-xs"></div>Mercado Pago ({Math.round(pctMP)}%)</div>
              <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-violet-500 shadow-xs"></div>Transferencias ({Math.round(pctTransf)}%)</div>
            </div>
          </div>
        </div>
          ...
          <Card className={`${phantomCardClass} p-5 flex flex-col justify-between hover:border-[#FF7A00]/30 transition-colors group cursor-default`}>
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <div className="text-xs mt-1 font-medium text-zinc-400">Pedidos este mes</div>
              </div>
